    ...
    def plt_distributions(self):
        ...
        with self.plot('distributions/conditions_per_patient') as (_, ax):
            ...
            for is_cured in [True, False]:
                conds_per_patient[is_cured].plot.hist(alpha=0.5, bins=range(conds_per_patient[is_cured].max()+1), ax=ax, density=True)
        ...
        with self.plot('distributions/trials_per_patient') as (_, ax):
            ...
            trials_per_patient.plot.hist(alpha=0.5, bins=range(trials_per_patient.max()+1), ax=ax, density=True)
        ...
        with self.plot('distributions/trials_per_condition') as (_, ax):
            ...
            trials_per_condition.plot.hist(alpha=0.5, bins=range(trials_per_condition.max()+1), ax=ax, density=True)
        ...
        with self.plot('distributions/success_scores') as (_, ax):
            ...
            trials_success.plot.hist(alpha=0.5, bins=100, ax=ax, density=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', '-d', type=str, help='path to the dataset file.')
