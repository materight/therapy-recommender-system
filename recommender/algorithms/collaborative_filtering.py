...
class CollaborativeFilteringRecommender(BaseRecommender):
    def __init__(self, approach: str, similarity: str, n_neighbors: int):
        """
        Collaborative filtering recommender.

        Args:
            approach (str): Type of collaborative filtering. Can be either 'user-user' or 'item-item'.
    ...
    def fit(self, dataset):
        ...
        # Compute the utility matrix, using as value the 'successful' column. 
        # The result is a NxM utility matrix, where (in case approach='user-user'):
        # - N is the number of conditions of each patient (i.e. the "users")
        # - M is the number of available therapies (i.e. the "items")
        # If approach='item-item', the utility matrix is transposed.
        self.utility = self._get_utility_matrix(dataset.p_trials, dataset.therapies)
        if self.approach == 'item-item':
            self.utility = self.utility.T
    ...
    def _get_top_similar_k(self, user_id: str):
        """Compute the k most similar conditions to the given one."""
        # Get trials of condition_id
        target_features = self.utility[self.utility.index == user_id]
        other_features = self.utility[self.utility.index != user_id]
        if self.similarity == 'levenshtein' and self.approach == 'user-user':
            ...
            target_sequence = self._get_trials_sequences(self._get_trials(self.dataset.p_trials, [user_id]))
            ...
            raise ValueError(f'Similarity {self.similarity} with {self.approach} approach is not supported.')
    ...
    def _predict_ratings(self, user_id: str, top_k_similarities: pd.DataFrame):
        ...
        pred_ratings = pred_ratings + self.global_baseline.loc[user_id]
        pred_ratings = pred_ratings.clip(0, 100)
        return pred_ratings

    def _predict_user_user(self, patient_id: str, condition_id: str):
         # Get top-k similar conditions
        top_k_similarities = self._get_top_similar_k(condition_id)
        # Compute ratings predictions
        condition_pred_ratings = self._predict_ratings(condition_id, top_k_similarities)
        return condition_pred_ratings

    def _predict_item_item(self, patient_id: str, condition_id: str):
        condition_pred_ratings = pd.Series(index=self.utility.index)
        for therapy_id in self.utility.index:
            # Get top-k similar therapies
            top_k_similarities = self._get_top_similar_k(therapy_id)
            # Compute ratings predictions
            therapy_pred_ratings = self._predict_ratings(therapy_id, top_k_similarities)
            condition_pred_ratings.loc[therapy_id] = therapy_pred_ratings.loc[condition_id]
        return condition_pred_ratings

    def predict(self, patient_id: str, condition_id: str):
        if self.approach == 'user-user':
            return self._predict_user_user(patient_id, condition_id)
        elif self.approach == 'item-item':
            return self._predict_item_item(patient_id, condition_id)
        else:
